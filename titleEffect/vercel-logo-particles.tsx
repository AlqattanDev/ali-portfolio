...
import { ASCII_TEXT } from './ascii-text'
...
export default function Component() {
  ...
  useEffect(() => {
    ...
    function createTextImage() {
      ...
      // ASCII text dimensions
      const fontSize = isMobile ? 8 : 16
      ctx.font = `${fontSize}px monospace`
      const textMetrics = ctx.measureText('ALI ALQATTAN')
      const textWidth = textMetrics.width
      
      const logoSpacing = isMobile ? 30 : 60
      const totalWidth = vercelLogoWidth + textWidth + logoSpacing
      ...
      // Draw ASCII text
      ctx.save()
      ctx.translate(vercelLogoWidth + logoSpacing, logoHeight / 2)
      ctx.font = `${fontSize}px monospace`
      ctx.textAlign = 'left'
    ...
    function createParticle(scale: number) {
      ...
      for (let attempt = 0; attempt < 100; attempt++) {
        ...
        if (data[(y * canvas.width + x) * 4 + 3] > 128) {
          ...
          const isTextLogo = x >= centerX + (totalWidth / 2) - textWidth
          return {
            x: x,
            y: y,
            baseX: x,
            ...
            color: 'white', 
            scatteredColor: isTextLogo ? '#FF6B6B' : '#00DCFF', 
            isAWS: isTextLogo,
